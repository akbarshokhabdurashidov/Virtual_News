...
import com.shakhrashidov.virtual_news.adapters.SearchRecAdapter
import com.shakhrashidov.virtual_news.util.*
import com.shakhrashidov.virtual_news.util.Constants.SEARCH_DELAY_TIME
    ...
    private fun observeSearchNews() {
        viewModel.searchNews.observe(viewLifecycleOwner, Observer {
            when (it) {
                ...
                is Resources.Error -> {
                    progress_bar.hide()
                    requireContext().toast(R.string.not_found.toString())
                    viewModel.searchTopBarState.postValue(TopBarState.NormalState())
                }
                else -> {
                }
    ...
    private fun observeCategoryNews() {
        viewModel.categoryNews.observe(viewLifecycleOwner, Observer {
            when (it) {
                ...
                is Resources.Error -> {
                    progress_bar.hide()
                    requireContext().toast(R.string.on_error.toString())
                    viewModel.searchTopBarState.postValue(TopBarState.NormalState())
                }
                else -> {
                }
